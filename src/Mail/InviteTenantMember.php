...
class InviteTenantMember extends Mailable
{
    ...
    public function envelope(): Envelope
    {
        ...
        ConfigHelper::getNotificationConfig('invite_email_from_name', config('mail.from.name'));

        return new Envelope(
            from: $fromAddress,
            subject: __('filament-member::default.mail.invite_subject', ['tenant' => $tenantName, 'app' => config('app.name')]),
        );
    }
    ...
    public function content(): Content
    {
        ...
        if ($this->invite->role instanceof $enumClass) {
            $roleLabel = method_exists($this->invite->role, 'getLabel')
